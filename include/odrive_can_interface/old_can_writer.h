...
#ifndef ODRIVECANINTERFACE_INCLUDE_ODRIVE_CAN_INTERFACE_OLD_CAN_WRITER_H_
#define ODRIVECANINTERFACE_INCLUDE_ODRIVE_CAN_INTERFACE_OLD_CAN_WRITER_H_
...
#include "can_id_enums.h"
#include "odrive_enums.h"

class OldDriveCanInterface {
 public:
  OldDriveCanInterface() {
...
//    }
    ...
    setStateFrame.can_id = can_id_ << 5 | static_cast<int>(ODriveCAN::ID::SET_AXIS_REQUESTED_STATE); // Example CAN ID for setting axis state, adjust as necessary
...
//    setStateFrame.data[0] = 0x08; // Example payload value for closed loop control, adjust as necessary
    uint32_t state = static_cast<uint32_t>(ODriveEnums::AxisState::AXIS_STATE_CLOSED_LOOP_CONTROL);; // Assuming 8 is the value for AXIS_STATE_CLOSED_LOOP_CONTROL
...
// Prepare the CAN frame
    ...
    velocityCommandFrame.can_id = (can_id_ << 5) | static_cast<int>(ODriveCAN::ID::SET_INPUT_VEL); // Combine your function code and node ID
...
// Write the frame to the CAN socket
    ...
    setStateFrame.can_dlc = 4;    // Data length: Assuming 4 byte is enough to command the state change
//    setStateFrame.data[0] = 0x08; // Example payload value for closed loop control, adjust as necessary
    state = static_cast<uint32_t>(ODriveEnums::AxisState::AXIS_STATE_IDLE );; // Assuming 8 is the value for AXIS_STATE_CLOSED_LOOP_CONTROL
    memcpy(setStateFrame.data, &state, sizeof(state)); // This ensures all 4 bytes are set correctly
...
#endif //ODRIVECANINTERFACE_INCLUDE_ODRIVE_CAN_INTERFACE_OLD_CAN_WRITER_H_
