...
//    }
  ...
  setStateFrame.can_id = can_id << 5 | static_cast<int>(ODriveCAN::ID::SET_AXIS_REQUESTED_STATE); // Example CAN ID for setting axis state, adjust as necessary
...
// Prepare the CAN frame
  ...
  velocityCommandFrame.can_id = (can_id << 5) | static_cast<int>(ODriveCAN::ID::SET_INPUT_VEL); // Combine your function code and node ID
